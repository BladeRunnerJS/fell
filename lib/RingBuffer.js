 ...
 *
 * @param size {Number} The maximum size of this window. This must be an integer larger than 0.
 * @constructor
 */
 ...
 *
 * @param {Object} object an item to add into this window.
 * @return the item that was pushed out of the window or null if the window is not full. Note, this
...
RingBuffer.prototype.forEach = function (func) {
	if (typeof func != 'function') {
		throw new TypeError(msg("parameter not function", typeof func));
	}
...
/**
 * @return {String} Returns a string representation of this RingBuffer.  This is intended to be human readable for debugging and may change.
 */
...
RingBuffer.prototype._checkSize = function(size) {
	if (size !== (size|0)) {
		throw new Error(msg("size not integer", size));
	}
	if (size < 1) {
		throw new Error(msg("size less than 1", size));
	}
};
